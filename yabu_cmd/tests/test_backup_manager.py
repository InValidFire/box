...
class TestBackupManager:
        ...
        for preset in presets:
            for backup in backup_manager.create_backups(preset, False, False):
                ...
                if preset.name == "testFile":
                    ...
                    assert str(backup.target) == str(Path("temp/folder/file.txt").absolute()) # allows Path and its children to equate. :)
    ...
    def test_create_backups_zip_force_no_keep(self, setup_create_backups):
        preset_manager = PresetManager(setup_create_backups)
        presets = preset_manager.get_presets()
        backup_manager = BackupManager()
        for i in range(4):
